...
const (
	...
	// file backend.
	FileBackendPasswordEnv = "GMAIL_VISIBILITY_MANAGER_KEYRING_PASSWORD"
	keyringOpenTimeout     = 5 * time.Second
	tokenRefreshTimeout    = 30 * time.Second
)

var ErrTokenNotFound = errors.New("gmail visibility manager token not found")

// TokenStore persists one OAuth token per instance/account.
...
// OpenTokenStore opens the configured token store.
func OpenTokenStore(cfg config.AuthStoreConfig) (TokenStore, error) {
	serviceName := strings.TrimSpace(cfg.Service)
	if serviceName == "" {
		serviceName = "gmail-visibility-manager"
	}
	ringCfg := keyring.Config{ServiceName: serviceName}
...
func openKeyring(cfg keyring.Config, useTimeout bool) (keyring.Keyring, error) {
	if useTimeout && runtime.GOOS == "linux" {
		...
		case <-time.After(keyringOpenTimeout):
			return nil, fmt.Errorf("open keyring timed out after %s; on Linux, try auth_store.backend=file", keyringOpenTimeout)
		}
	}
	ring, err := keyring.Open(cfg)
	if err != nil {
...
// Save stores the token under the manager's instance/account key.
...
// Load reads the stored token for the manager instance/account.
...
// TokenSource returns a refresh-capable token source that persists updates.
